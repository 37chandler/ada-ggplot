...
ggplot(gd, aes(x=ownerSales/1000,y=departmentName)) + 
  ...
  scale_x_continuous(breaks=c(0,30,60,90,120), labels=c('$0','$3k','$6k','$9k','$12k')) +
  xlab("Owner Sales (Divided by 10)") + 
  ylab("Department Name") +
  labs(title="Sales by Department") + 
  theme(plot.title = element_text(hjust=0.5)) # Centers the title, rather than it being left-aligned
...
ggplot(gd, aes(ownerSales, ..count..)) +
  ...
  scale_x_continuous(name='sales',breaks=c(0,30000,60000,90000,120000), labels=c('0','30k','60k','90k','120k'))

```

## Problem 5 - violin plots!
...
ggplot(gd, aes(ownerSales, departmentName)) +
  geom_violin(scale='width',trim=FALSE,draw_quantiles=c(0.25,0.5,0.75)) + 
  geom_vline(xintercept=mean(gd$ownerSales),color='red') +
  annotate(geom='text', x=57500, y='FLOWERS',label='Overall Mean Sales ($28,141.53)',color='red')
